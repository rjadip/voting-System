...
//	$test 	= mysqli_query($connect, "SELECT * FROM tb_vote;");
	$select = mysqli_query($connect, "SELECT nama_team, count(id_team) AS ratings FROM tb_team INNER JOIN tb_vote on tb_team.id = tb_vote.id_team GROUP BY id_team ORDER BY 'ratings' DESC;");	
